...
function find_heuristic(g, h) {
  g = +g;

  return [g + h];
}

function A_star(start, matrix, goal) {
  ...
  while (stack.length != 0) {
    ...
    for (let i = 0; i < 4; i += 1) {
      ...
      let h = Math.round(Math.sqrt((adjx - xn) ** 2 + (adjy - yn) ** 2));
      // let dist = Math.abs(matrix[adjx][adjy] - matrix[r][c]);
      let temp1 = find_heuristic(g + 1, h);
      // console.log(g, h, temp1);

      temp.push([temp1, [adjx, adjy]]);
...
export { find_coordinates, A_star };
